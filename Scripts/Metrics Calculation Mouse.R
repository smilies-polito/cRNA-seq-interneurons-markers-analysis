...
mouse_CDS <- readRDS("C:/Users/loren/single cell/Processed CDS/allen_human_ACC")
mouse_CDS@colData@listData[["clusters"]] <- mouse_CDS@clusters@listData[["UMAP"]][["clusters"]]
mouse_CDS@colData@listData[["partition"]] <- mouse_CDS@clusters@listData[["UMAP"]][["partitions"]]


############# one needs to select the interneurons cells
mouse_CDS_sub <- choose_cells(mouse_CDS)
...
####### create a CDS which is a subset of the original one with only the markers we are considering
GG <- mouse_CDS[row.names(subset(rowData(mouse_CDS),gene_short_name %in% c("Gad1", "Gad2"))),]
...
Gad_ing_Mouse <- association
Gad_scores_Mouse <- cluster_marker_score_table
Gad_scores_Mouse_spec <- Gad_scores_Mouse %>%
...
ALH <- mouse_CDS_sub[row.names(subset(rowData(mouse_CDS_sub),gene_short_name %in% c("Adarb2", "Htr3a", "Lhx6"))),]
...
MGE_inf_Mouse <- association
MGE_score_Mouse <- cluster_marker_score_table
MGE_score_spec_Mouse <- cluster_marker_score_table
MGE_score_spec_Mouse <- MGE_score_spec_Mouse %>%
...
PSVL <- mouse_CDS_sub[row.names(subset(rowData(mouse_CDS_sub),gene_short_name %in% c("Pvalb", "Sst", "Vip", "Lamp5"))),]
...
PSVL_inf_Mouse <- association
PSVL_score_Mouse <- cluster_marker_score_table 
PSVL_score_Mouse <- PSVL_score_Mouse %>%
  top_n(4, marker_score)
############################


############# put all together in one table
INF_Mouse = rbind(GAD_inf_Mouse, MGE_inf_Mouse, PSVL_inf_Mouse)
score_Mouse <- rbind(Gad_scores_Mouse_spec, MGE_score_spec_Mouse, PSVL_score_Mouse)
